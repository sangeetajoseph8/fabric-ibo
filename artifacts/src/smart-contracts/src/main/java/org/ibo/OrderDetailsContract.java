...
@Contract(name = "OrderDetailsContract",
        info = @Info(title = "OrderDetails contract",
                description = "OrderDetails Contract",
                version = "0.0.1",
                license = @License(name = "Apache-2.0", url = ""),
                contact = @Contact(email = "smart-contracts@example.com",
                        name = "smart-contracts",
                        url = "http://smart-contracts.me")))
@Default
public class OrderDetailsContract implements ContractInterface {
    ...
    @Transaction()
    public OrderDetailsList getAllOrderForOrgName(Context ctx, String orgName, int pageSize, String bookmark, int g) {
        ...
        for (KeyValue kv : queryResultIterator) {
            System.out.println(OrderDetails.fromJSONString(new String(kv.getValue(), UTF_8)).toJSONString());
            orderDetailsList.getOrders().add(OrderDetails.fromJSONString(new String(kv.getValue(), UTF_8)));
        }
